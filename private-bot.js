...
app.post('/webhook-callback', async function (req, res) {
  ...
  if (validationToken) {
    ...
    if (req.body.ownerId == body.creatorId) {
      ...
      // Treat message as JQL
      try {
        console.log("Executing JQL:", body.text);
        const jiraResults = await jiraClient.searchIssues(body.text);
        await sendReportResults(body.groupId, jiraResults);
        // Re-send the form card after the report
        await send_card(body.groupId, make_jql_form_card());
      } catch (e) {
        console.error("Jira Error:", e.message);
        send_message(body.groupId, `Error executing JQL: ${e.message}`);
      }
    }
    ...
    console.log('Bot joined a group, sending JQL form card');
    send_card(req.body.body.id, make_jql_form_card());
  } else {
...
app.post('/user-submit', async function (req, res) {
  ...
  res.status(200).end();
  var body = req.body
  if (body.data.path == 'generate-report') {
    const productTrack = (body.data.productTrack || 'IVA').trim();
    const targetQuarter = (body.data.targetQuarter || '2026-Q1').trim();
    // Escape double quotes inside the value, then wrap in double quotes for JQL safety
    const safePT = '"' + productTrack.replace(/"/g, '\\"') + '"';
    // For labels, replace spaces with underscores and quote for JQL safety
    const safeLabel = '"' + productTrack.replace(/ /g, '_').replace(/"/g, '\\"') + '"';
    const ptFilter = `("Product Track" in (${safePT}) OR labels in (${safeLabel}))`;
    // Exclude ENG categories if toggle is on (default: true)
    const excludeEng = body.data.excludeEngCategories !== 'false';
    const engFilter = excludeEng ? ' AND "Primary category" not in ("ENG - Maintenance / KTLO", "ENG - Modernization", "ENG - Research")' : '';
    console.log(`Card submit: productTrack=${productTrack}, targetQuarter=${targetQuarter}, excludeEng=${excludeEng}`);
    const jql = `project in (INIT) AND ${ptFilter} AND "Target Delivery Quarter" in (${targetQuarter}) AND (Resolution not in (Cancelled, Duplicate) OR Resolution is EMPTY)${engFilter} ORDER BY "PT Rank" ASC`;
    try {
      ...
      console.error("Jira Error (card):", e.message);
      send_message(body.conversation.id, `Error executing JQL: ${e.message}`);
    }
  } else if (body.data.path == 'new-card') {
  ...
  if (jiraResults && jiraResults.length > 0) {
    ...
    for (const issue of jiraResults) {
      ...
      if (currentBatch && (currentBatch.length + separator.length + issue.length) > MAX_MSG_LEN && currentBatch !== header) {
        ...
        currentBatch += separator + issue;
      }
    }
    if (currentBatch) batches.push(currentBatch);

    for (const batch of batches) {
      await send_message(groupId, batch);
    }
  } else {
    await send_message(groupId, "No issues found or empty response.");
  }
}

// Build the JQL form adaptive card
function make_jql_form_card() {
  return {
    type: "AdaptiveCard",
    ...
    body: [
      ...
      {
        type: "TextBlock",
        text: "Fill in the fields below and click Generate Report.",
        wrap: true
      },
      {
        type: "TextBlock",
      ...
      {
        type: "TextBlock",
        text: "🚫 Exclude Engineering categories:",
        weight: "Bolder",
        spacing: "Large"
      },
      {
        type: "TextBlock",
        text: "• ENG - Maintenance / KTLO\n• ENG - Modernization\n• ENG - Research",
        wrap: true,
        size: "Small",
        isSubtle: true
      },
      {
        type: "Input.Toggle",
        id: "excludeEngCategories",
        title: "Hide these from report",
        value: "true"
      },
      {
        type: "ActionSet",
